import { NextRequest, NextResponse } from 'next/server'
import { supabase } from '@/lib/supabase'

// GET /api/escalations - Query escalations
export async function GET(request: NextRequest) {
  try {
    const searchParams = request.nextUrl.searchParams
    const agent_id = searchParams.get('agent_id')
    const status = searchParams.get('status')
    const priority = searchParams.get('priority')
    const limit = searchParams.get('limit') || '50'

    let query = supabase.from('escalations').select(`
      *,
      agent:agents(id, name, framework),
      workflow:workflows(id, name, version),
      execution:workflow_executions(id, status)
    `)

    if (agent_id) {
      query = query.eq('agent_id', agent_id)
    }
    if (status) {
      query = query.eq('status', status)
    }
    if (priority) {
      query = query.eq('priority', priority)
    }

    query = query
      .order('priority', { ascending: false })
      .order('created_at', { ascending: false })
      .limit(parseInt(limit))

    const { data, error } = await query

    if (error) throw error

    return NextResponse.json({ data })
  } catch (error) {
    console.error('Escalations GET error:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
  }
}

// POST /api/escalations - Create an escalation
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const {
      agent_id,
      workflow_id,
      execution_id,
      reason,
      context,
      priority = 'medium'
    } = body

    if (!agent_id || !reason || !context) {
      return NextResponse.json(
        { error: 'agent_id, reason, and context are required' },
        { status: 400 }
      )
    }

    // Validate priority
    const validPriorities = ['low', 'medium', 'high', 'urgent']
    if (!validPriorities.includes(priority)) {
      return NextResponse.json(
        { error: `Invalid priority. Must be one of: ${validPriorities.join(', ')}` },
        { status: 400 }
      )
    }

    // Validate agent exists
    const { data: agent } = await supabase
      .from('agents')
      .select('id, name')
      .eq('id', agent_id)
      .single()

    if (!agent) {
      return NextResponse.json(
        { error: 'Agent not found' },
        { status: 404 }
      )
    }

    const { data, error } = await supabase
      .from('escalations')
      .insert({
        agent_id,
        workflow_id,
        execution_id,
        reason,
        context,
        priority,
        status: 'open'
      })
      .select()
      .single()

    if (error) throw error

    // Log escalation creation
    await supabase.from('audit_log').insert({
      agent_id,
      action: 'escalation_create',
      resource_type: 'escalation',
      resource_id: data.id,
      details: { reason, priority, agent_name: agent.name }
    })

    return NextResponse.json({ data }, { status: 201 })
  } catch (error) {
    console.error('Escalations POST error:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
  }
}
